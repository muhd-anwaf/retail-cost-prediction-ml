...
Linear models (Lasso, Ridge) were ruled out early — EDA showed near-zero Pearson correlations between all features and `cost`, meaning the relationships are non-linear and likely involve feature interactions.

`HistGradientBoostingRegressor` was the right fit because:
...
- Natively supports missing values — no imputation needed
- Robust to outliers and skewed features

**Baseline: `RandomForestRegressor`** — used to validate that boosting provides a meaningful lift.
